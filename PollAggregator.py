...
def plotEstimates():
    ...
    for d in demE:
        ...
        line, = ax.plot(demSmooth[0], demSmooth[1], linewidth=1.75)
    ...
    plt.legend(demLinesSorted, demLabelsSorted, frameon=False, bbox_to_anchor=(.5,-.12), borderaxespad=0.5, loc='center', ncol=4)
    #Make the title of the plot look like "The Democratic Primary Field: May 03-Jun 02"
    plt.title('The Democratic Primary Field: %(begindate)s-%(enddate)s' %{"begindate":beginDate.strftime('%b %d'), "enddate":endDate.strftime('%b %d')}, fontsize=12)
    #Make the lables of the x-axis only occur every 5 days
    xDates = [beginDate + datetime.timedelta(5*i) for i in range(1,6)]
    xDates.append(beginDate)
    xDates.append(endDate)
    xDates = sorted(xDates)
    xDateLabels = [d.strftime('%b %d') for d in xDates]
    plt.xticks(xDates, xDateLabels)
    #Save the chart in Charts with a filename that includes the timestamp of the latest estimate.
    plt.figtext(.1, .6, 'ElectoralStatistics.com', color="#283D4B")
    ...
    for r in repubE:
        ...
        line, = ax.plot(repubSmooth[0], repubSmooth[1], linewidth=1.5)
    ...
    plt.legend(repubLinesSorted, repubLabelsSorted, frameon=False, bbox_to_anchor=(.5,-.2), borderaxespad=0.5, loc='center', ncol=4)
    plt.title('The Republican Primary Field: %(begindate)s-%(enddate)s' %{"begindate":beginDate.strftime('%b %d'), "enddate":endDate.strftime('%b %d')}, fontsize=12)
    xDates = [beginDate + datetime.timedelta(5*i) for i in range(1,6)]
    xDates.append(beginDate)
    xDates.append(endDate)
    xDates = sorted(xDates)
    xDateLabels = [d.strftime('%b %d') for d in xDates]
    plt.xticks(xDates, xDateLabels)
    plt.figtext(.1, .485, 'ElectoralStatistics.com', color="#283D4B")
